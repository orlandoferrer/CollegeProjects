...
infor:	bge $t5, $t6, endinfor	#out of inner for when i>= size
	...
	mul $s7, $s0, 4		#(i+off1) *4 is the offset
	move $s0, $s7
	la $s1, mat1
	add $s7, $s1, $s0	#$s2 = mat1[0] + offset
	move $s2, $s7
	lw $s2, 0($s2)		#mat1[i+off1]

	mul $s7, $t5, $t6	#$s4 = i*size
	move $s4, $s7
	add $s4, $s4, $t8	#$s4 = (i*size) + off2
	mul $s7, $s4, 4		#$s4 = $s4 * 4  :real offset
	move $s4, $s7
	la $s1, mat2
	add $s5, $s1, $s4	#$s5 = mat2[0] + offset
	lw $s5, 0($s5)		#$s5 = mat2[off2+i * size]

	mul $s7, $s2, $s5	#mat1[1+off] * mat2[off+i*size]
	move $s3, $s7
...
endoutfor:
	...
	mul $s7, $t5, 0	#reset i=0 as counter
	move $t5, $s7
